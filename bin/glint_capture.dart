...
/// CLI for Glint Capture — generates store-ready screenshots.
///
/// Usage:
///   dart run glint_capture init      # create glint.yaml + sample screens
///   dart run glint_capture           # generate screenshots from config
///   dart run glint_capture --devices pixel7,galaxy_s23
///   dart run glint_capture --output build/screenshots
Future<void> main(List<String> args) async {
  if (args.isEmpty) {
    await _capture(_parseArgs(args));
    return;
  }
...
Future<void> _init() async {
  ...
  print('''
Done! Next steps:
  1. Edit glint.yaml to configure devices and output
  2. Edit $screensPath to define your screens
...
Future<void> _capture(_Options options) async {
  ...
  if (configPath == null) {
    print('Error: No glint.yaml found. Run: dart run glint_capture init');
    exit(1);
  }
  ...
  final outputDir = options.outputDir ?? config.outputDir;
  ...
  if (testFile == null) {
    print('\nError: No screens file found.');
    print('Run: dart run glint_capture init');
    exit(1);
  }

  print('\n  Screens: ${p.relative(testFile)}');
  print('\nGenerating screenshots...');

  // Run flutter test with glint tag
...
String? _findTestFile() {
  ...
  // Search test/ for any file calling glintScreenshots
...
class _Options {
  _Options({this.config, this.outputDir, this.devices});
...
_Options _parseArgs(List<String> args) {
  ...
  return _Options(config: config, outputDir: outputDir, devices: devices);
}
...
Usage:
  dart run glint_capture [command] [options]

Commands:
  (none)              Generate screenshots from config
...
Workflow:
  1. dart run glint_capture init
  2. Edit glint.yaml — set app name, devices, output
  3. Edit test/glint_screenshots_test.dart — define screens
  4. dart run glint_capture
  5. Import build/glint_screenshots/ into Glint-Web
''');
}

const _defaultConfig = '''# Glint Capture configuration
# Docs: https://github.com/darkmintis/Glint-Capture

app_name: MyApp
tagline: "Your app tagline"
output: build/glint_screenshots
store: play  # play | ios
